...
as.named.vector.table <- function(table, verbose = TRUE,
  ...
  v <- c(table) # Atomic named vector, stripped of the table's class/dim attributes
...
combine.matrices.by.rowname.intersect <- function(matrix1, matrix2, k = 2) { # combine matrices by row name intersection
  ...
  Stringendo::iprint(length(idx), "out of", substitute(matrix1), length(rn1), "and", length(rn2), substitute(matrix2), "rownames are merged")
